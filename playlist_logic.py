...
def normalize_text(value: str, lowercase: bool = False) -> str:
    """Trim a text value, optionally lowercasing it for comparison."""
...
def normalize_song(raw: Song) -> Song:
    ...
    artist = normalize_text(str(raw.get("artist", "")), lowercase=True)
    genre = normalize_text(str(raw.get("genre", "")), lowercase=True)
...
def classify_song(song: Song, profile: Dict[str, object]) -> str:
    ...
    # Combine the mood-signal fields into one lowercased string to search.
    keyword_text = " ".join([genre] + [str(t) for t in tags]).lower()

    is_hype_keyword = any(k in keyword_text for k in hype_keywords)
    is_chill_keyword = any(k in keyword_text for k in chill_keywords)
